...
app.post("/room/create", (req, res) => {
  ...
  if (idExists !== undefined) {
    return res
      .status(400)
      .json({ message: "room already exists.", RoomsList: room });
  } else {
    room.push(addRoom);
    res
      .status(201)
      .json({ message: "room created", RoomsList: room, added: addRoom });
  }
});
...
app.get("/booking/all", function (req, res) {
  ...
  } else res.status(200).json({ BookingList: booking });
});
...
app.post("/booking/create/:id", (req, res) => {
  try {
    ...
    if (matchID.length > 0) {
      ...
      return res
        .status(201)
        .json({ message: "hall booked", Bookings: booking, added: newbooking });
    }
  } catch (err) {
    res
      .status(400)
      .json({ message: "error booking room", error: err, data: booking });
  }
});
...
app.get("/customers/find/:name", function (req, res) {
  ...
  let customer = booking.filter(
    (el) => el.customer.toLowerCase() === name.toLowerCase()
  );
  if (customer.length > 0) {
    ...
    res
      .status(200)
      .json({ No_of_bookings: customer.length, list_bookings: customer });
  } else {
    res
      .status(400)
      .json({ message: "No bookings found for the specified customer name" });
  }
});

app.listen(4000, () => console.log("started server 4000 hallbooking"));
